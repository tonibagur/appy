...
BAD_STATEMENT_GROUP = 'Syntax error while parsing a note whose content is ' \
  '"%s". In a note, you may specify at most 2 lines: a pod statement and a ' \
  '"from" clause. ' + POD_STATEMENT + ' ' + FROM_CLAUSE
...
NULL_ACTION_ERROR = 'There was a problem with this action. Possible causes: ' \
  '(1) you specified no action (ie "do text") while not specifying any from ' \
  'clause; (2) you specified the from clause on the same line as the action, ' \
  'which is not allowed (ie "do text from ...").'
    ...
    def createAction(self, statementGroup):
        ...
        try:
            ...
            if len(statementGroup) > 1:
                ...
                source = 'from'
                fromInfo = MemoryBuffer.fromRex.match(fromClause)
                if not fromInfo:
                ...
                if isinstance(evalEntry, Expression):
                    try:
                        ...
                        if not self.env.raiseOnError:
                            ...
                            raise EvaluationError(e, EVAL_EXPR_ERROR % \
                                        (evalEntry.expr, '\n'+Traceback.get(5)))
