...
class UserWrapper(AbstractWrapper):
    ...
    def showEmail(self):
        '''In most cases, email is the login. Show the field only if it is not
           the case.'''
        # If the login is an email, dont show the "email" field
        email = self.login
        if email and String.EMAIL.match(email): return
        # Display the email (read-only if from LDAP)
        if (self.source != 'zodb'): return ('view', 'result')
        return True
    ...
    def showRoles(tool):
        ...
        if user.has_role('Owner', user): return 'view'
