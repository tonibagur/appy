...
class Ref(Field):
    ...
    pxObjectTitle = Px('''
     <x var="navInfo=field.getNavInfo(zobj, startNumber + currentNumber, \
      ...
      <x if="not selector">::tied.o.getSupTitle(navInfo)</x>
      ...
      <x if="not selector">
       <span style=":showSubTitles and 'display:inline' or 'display:none'"
            name="subTitle" class="discreet" var="sub=tied.o.getSubTitle()"
            if="sub">::sub</span>
      </x></x>''')
    ...
    pxViewList = Px('''
     <div id=":ajaxHookId">
      ...
      <table if="objects" id=":collapse.id" style=":collapse.style"
             class=":ztool.getResultCss(tiedClassName)"
    ...
    pxViewMinimal = Px('''<x>::field.renderMinimal(obj, objects)</x>''')

    # Simplified widget for fields with render="links"
    pxViewLinks = Px('''<x if="not objects">_('no_ref')</x>
     <x for="o in objects">
      <a href=":o.url">:field.getReferenceLabel(obj, o, True)</a><x
       if="not loop.o.last">, </x></x>''')
    ...
    def __init__(self, klass=None, attribute=None, validator=None,
      ...
      addConfirm=False, delete=None, noForm=False, link=True, unlink=None,
      ...
      group=None, layouts=None, showHeaders=False, shownInfo=None, select=None,
      maxPerPage=30, move=0, indexed=False, mustIndex=True, searchable=False,
      ...
      xml=None, showActions=True, collapsible=False, links=True,
      emptyIndexValue=''):
        ...
        # a composite object and tied object(s) will its components.
        ...
        self.select = select
        if isinstance(select, Search): select.name = '_field_'
            ...
            if self.masterValue and callable(self.masterValue):
                ...
                if not self.select:
                    ...
                    if isinstance(search, Search):
                        ...
                        # self.select has returned objects
                        objects = search
    ...
    def getMenuUrl(self, zobj, tied):
        '''We must provide the URL of the p_tied object, when shown in a Ref
           ...
           impossible.'''
        target = Object(target='_self', openPopup=False)
        if self.menuUrlMethod:
            ...
            if isinstance(res, str):
                ...
                target.target = res[1]
                return res[0], target
        return tied.o.getUrl(nav=''), target
    ...
    def getBatchInfo(self, render, req, hookId):
        ...
        if render == 'menus': return None, None
    ...
    def getReferenceLabel(self, obj, refObject, unlimited=False, dir='ltr'):
        ...
        shownInfo = self.getAttribute(obj, 'shownInfo')
        tool = refObject.tool.o
        className = tool.getPortalType(refObject.klass)
        for col in tool.getColumnsSpecifiers(className, shownInfo, dir):
            name = col.field.name
        ...
        if rq['semantics'] == 'checked':
            ...
            if self.link == 'popup':
                ...
                brains = obj.o.executeQuery(className, search=self.select,
                    ...
                    filterValue=rq.get('filterValue'))
