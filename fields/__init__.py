...
class Field:
     ...
     <x if="field.name == 'title'"
      ...
      <x if="mayView"
       ...
       <span style=":showSubTitles and 'display:inline' or 'display:none'"
             name="subTitle" var="sub=zobj.getSubTitle()" class="discreet"
    ...
    def inGrid(self):
        ...
        if isinstance(self.group, __builtins__['dict']):
            for group in self.group.itervalues():
                if not group or (group.style != 'grid'): return
            return True
    ...
    def computeDefaultLayouts(self):
        '''This method gets the default layouts from a Field or a copy from the
           global default field layouts when they are not available.'''
        res = self.getDefaultLayouts()
        if not res:
            res = defaultFieldLayouts(self) # Get the global default layouts
        return res
    ...
    def getStoredValue(self, obj, name=None, fromRequest=False):
        ...
        if '*' in name:
            ...
            if fromRequest: res = obj.REQUEST.get(name, None)
    ...
    def getInlineEditableValue(self, obj, value):
        ...
        if obj.allows('write') and self.getAttribute(obj, 'inlineEdit'):
            ...
            return '<span class="editable" onclick="askField(\'%s\',\'%s\',' \
                   '\'edit:cell\')">%s</span>' % (hook, obj.url, value)
        return value
